...
class TerminalMessanger(Messages):
    def send_to_user(self, massage: str) -> None:
        print(massage)


class WebMessanger(Messages):
    def send_to_user(self, massage: str) -> None:
...
def main():
    ...
    while meander is None:
        word_question = input(f'Choose interface: 1 = Terminal, 2 = Web \n')
        if word_question == '1':
            meander = TerminalMessanger()
        elif word_question == '2':
            meander = WebMessanger()
        else:
    ...
    while True:
        try:
            ...
            if command in ["close", "exit"]:
                meander.send_to_user("Good bye!")
                break
            elif command == "hello":
                meander.send_to_user("How can I help you?")

            elif command == "add":
                meander.send_to_user(add_contact(args, book))

            elif command == 'change':
                meander.send_to_user(change_contact(args, book))

            elif command == 'phone':
                meander.send_to_user(show_phone(args, book))

            elif command == 'all':
                meander.send_to_user(show_all(book))

            elif command == "add-birthday":
                meander.send_to_user(add_birthday(args, book))

            elif command == "show-birthday":
                meander.send_to_user(show_birthday(args, book))

            elif command == "birthday":
                meander.send_to_user(birthdays(book))

            elif command == 'see_command':
                meander.send_to_user(see_command())

            else:
                meander.send_to_user("Invalid command.")
